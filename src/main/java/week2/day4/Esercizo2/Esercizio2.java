...
public class Esercizio2 {
    public static void main(String[] args) {
        ...
        Map<Customer, Double> VenditeTotPerCliente = ordini.stream().collect(Collectors.groupingBy(Order::getCustomer, Collectors.summingDouble(order -> order.getProducts()
                .stream().mapToDouble(Product::getPrice).sum())));

        System.out.println("Totale delle vendite per ogni cliente:");
        VenditeTotPerCliente.forEach((customer, totalSales) -> System.out.println("Cliente: " + customer.getName() + ", Totale Vendite: " + totalSales));
    }
}
